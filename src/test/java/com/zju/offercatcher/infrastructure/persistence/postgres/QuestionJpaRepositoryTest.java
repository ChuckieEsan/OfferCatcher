...
class QuestionJpaRepositoryTest {
    ...
    void findByUserIdPaginated() {
        repo.save(makeEntity(1L, "q1", "user-1", "Q1", "阿里", "Java", null));
        repo.save(makeEntity(2L, "q2", "user-2", "Q2", "字节", "Go", null));

        List<QuestionJpaEntity> result = repo.findByUserIdPaginated("user-1", 10, 0);
        assertThat(result).hasSize(1);
        assertThat(result.getFirst().getQuestionHash()).isEqualTo("q1");
    }
    ...
    void findUnansweredQuestions() {
        repo.save(makeEntity(1L, "q1", "user-1", "Q1", "阿里", "Java", null));
        repo.save(makeEntity(2L, "q2", "user-1", "Q2", "字节", "Go", "已有答案"));
        repo.save(makeEntity(3L, "q3", "user-1", "Q3", "美团", "Python", ""));
    ...
    void findRecentlyUpdated() {
        ...
        repo.save(makeEntity(1L, "q1", "user-1", "Q1", "阿里", "Java", null));
        repo.save(makeEntity(2L, "q2", "user-1", "Q2", "字节", "Go", null));
    ...
    @Test
    @DisplayName("findByQuestionHash 应返回指定题目")
    void findByQuestionHash() {
        repo.save(makeEntity(1L, "q1", "user-1", "Q1", "阿里", "Java", null));

        Optional<QuestionJpaEntity> result = repo.findByQuestionHash("q1");
        assertThat(result).isPresent();
        assertThat(result.get().getCompany()).isEqualTo("阿里");
    }

    @Test
    @DisplayName("deleteByIdAndUserId 应删除且验证所有权")
    void deleteByIdAndUserId() {
        repo.save(makeEntity(1L, "q1", "user-1", "Q1", "阿里", "Java", null));

        int deleted = repo.deleteByIdAndUserId(1L, "user-1");
        assertThat(deleted).isEqualTo(1);
        assertThat(repo.findByQuestionHash("q1")).isEmpty();
    }

    @Test
    @DisplayName("deleteByIdAndUserId 不同用户不应删除")
    void deleteByOtherUser() {
        repo.save(makeEntity(1L, "q1", "user-1", "Q1", "阿里", "Java", null));

        int deleted = repo.deleteByIdAndUserId(1L, "user-2");
        assertThat(deleted).isEqualTo(0);
        assertThat(repo.findByQuestionHash("q1")).isPresent();
    }
}
